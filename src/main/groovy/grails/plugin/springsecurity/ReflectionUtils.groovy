...
class ReflectionUtils {
    ...
    // Grails 2.3+ only
    static UrlMappingInfo[] matchAllUrlMappings(UrlMappingsHolder urlMappingsHolder,
                                                String requestUrl,
                                                GrailsWebRequest grailsRequest,
                                                MimeTypeResolver mimeTypeResolver) {
        String method = grailsRequest.currentRequest.method
        String version = grailsRequest.getHeader(HttpHeaders.ACCEPT_VERSION) ?:
                mimeTypeResolver.resolveRequestMimeType(grailsRequest).version
